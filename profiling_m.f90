...
#define MPI_MYFLOAT MPI_DOUBLE_PRECISION
   ...
   type single_routine_timer_t
      ...
      real(kind=PRC) :: sqtiming
      real(kind=PRC) :: maxtiming !< cumulated timing
      real(kind=PRC) :: mintiming !< cumulated timing
   ...
   type routine_timer_t
      ...
      real(kind=PRC) :: maxtiming(mxroutine)  !< store max section timing
      real(kind=PRC) :: mintiming(mxroutine)  !< store min section timing
...
#endif
   logical, dimension(mxroutine), save :: ltiming_routine_i=.false.
   logical, dimension(mxroutine), save :: ltiming_routine_e=.false.

   real(kind=PRC), dimension(mxroutine) :: timing_routine
...
#else
   real(kind=PRC), save :: tstart, tend, tstart0
#endif
   integer :: iotiming
   logical :: lwritetime=.true.
...
#ifdef CUDA
   public :: print_memory_registration_cuda
   public :: get_memory_cuda
#endif
   public :: get_memory
   public :: get_totram
   public :: current_time
...
!***********************************************************************
      ...
      IF(myrank==0 .and. lwritetime)THEN
         OPEN(iotiming,FILE='time.dat')
         WRITE(iotiming,*) '# Number of MPI Processes:',nprocs
      ENDIF
      ...
      DO isec=1, SIZE(section_cum)
         ...
         section_cum(isec)%timer_routine%timing(:) = 0.0
         section_cum(isec)%timer_routine%maxtiming(:) = 0.0
         section_cum(isec)%timer_routine%mintiming(:) = huge(1.0)
         section_cum(isec)%timer_routine%icall(:) = 0

         section_blk(isec)%timer_routine%timing(:) = 0.0
         section_blk(isec)%timer_routine%maxtiming(:) = 0.0
         section_blk(isec)%timer_routine%mintiming(:) = huge(1.0)
         section_blk(isec)%timer_routine%icall(:) = 0

      ENDDO

      timer_standalone(1:mxroutine)%timing = 0.0
      timer_standalone(1:mxroutine)%sqtiming = 0.0
      timer_standalone(1:mxroutine)%icall = 0
...
!***********************************************************************
      ...
      DO isec=1, SIZE(section_cum)
         ...
         section_cum(isec)%timer_routine%timing(:) = 0.0 ! timings of routines
         section_cum(isec)%timer_routine%maxtiming(:) = 0.0
         section_cum(isec)%timer_routine%mintiming(:) = huge(1.0)
         ...
         section_blk(isec)%timer_routine%timing(:) = 0.0 ! timings of routines
         section_blk(isec)%timer_routine%maxtiming(:) = 0.0
         section_blk(isec)%timer_routine%mintiming(:) = huge(1.0)
...
!***********************************************************************
      ...
      REAL(kind=PRC), parameter :: mymax=real(1.d-6,kind=PRC)
...
!CLOSE(iotempo_partial)
      ...
      DO isec=1, nsection
         ...
         IF(myrank==0) THEN
            ...
            WRITE(ioout_l,1) 100.*section_blk(isec)%timing / MAX(mymax,total_time), &
...
!***********************************************************************
      ...
      if(myrank==0 .and. lwritetime)WRITE(iotiming,*)
      ...
      DO i=nsection,1,-1
         ...
         IF(myrank==0) THEN
            ...
            WRITE(ioout_l,1) 100.*section_cum(isec)%timing / MAX(mymax,total_time), &
            ...
            IF(lwritetime)&
               WRITE(iotiming,1) 100.*section_cum(isec)%timing / MAX(mymax,total_time), &
               TRIM(section_cum(isec)%name_sec), &
               section_cum(isec)%timing,' time/call[s],', &
               section_cum(isec)%icall,' calls'!, &
            !section_cum(isec)%icall / (real(ifreq_l,kind=PRC)),' calls/step'
...
!***********************************************************************
      ...
      REAL(kind=PRC) :: f,ff,tres,rate,stdev
      INTEGER :: i,j

      t_routine_max(1:nroutine) = timer_standalone(1:nroutine)%maxtiming
      t_routine_min(1:nroutine) = timer_standalone(1:nroutine)%mintiming
      ...
      IF(myrank==0) THEN
         ...
         write(ioout_l,'(a10,13x,a10,10x,a30,5x,2a12,1x,3a15)') &
            'Time Share', &
            'Section', &
            'Routine', &
            'Time/Call[s]', &
         ...
         DO j=nroutine+1,1,-1 ! ascending order
            ...
            IF(timer_standalone(i)%icall>0) ff = ff + t_routine_ave(i)

            stdev = real(1.d0,kind=PRC)/real(timer_standalone(i)%icall,kind=PRC)* &
               sqrt(abs(real(timer_standalone(i)%icall,kind=PRC)*timer_standalone(i)%sqtiming- &
               (timer_standalone(i)%timing)**2.d0))
...
!                       t_routine_max(i)/timer_standalone(i)%icall
            WRITE(ioout_l,1) t_routine_ave(i)*100. / t_routine_ave(nroutine+2), &
               TRIM(timer_standalone(i)%asection), &
               TRIM(timer_standalone(i)%aroutine), &
               t_routine_ave(i) / timer_standalone(i)%icall, &
               timer_standalone(i)%icall, &
!                       rate, &
               t_routine_min(i), &
               t_routine_max(i),stdev
            IF(lwritetime)&
               WRITE(iotiming,1) t_routine_ave(i)*100. / t_routine_ave(nroutine+2), &
               TRIM(timer_standalone(i)%asection), &
               TRIM(timer_standalone(i)%aroutine), &
               t_routine_ave(i) / timer_standalone(i)%icall, &
               timer_standalone(i)%icall, &
...
!***********************************************************************
      ...
      t_routine_max(1:section%nroutine) = &
         section%timer_routine%maxtiming(1:section%nroutine)
      t_routine_min(1:section%nroutine) = &
         section%timer_routine%mintiming(1:section%nroutine)
      ...
      IF(myrank==0) THEN
         ...
         DO j=section%nroutine+1,1,-1 ! ascending order
            ...
            f = f + t_routine_ave(i)*100. / t_routine_ave(section%nroutine+2)

            IF(section%timer_routine%icall(i)>0) ff = ff + t_routine_ave(i)

            WRITE(ioout_l,1) t_routine_ave(i)*100. / t_routine_ave(section%nroutine+2), &
               section%aroutine(i), &
               t_routine_ave(i) / section%timer_routine%icall(i), &
               section%timer_routine%icall(i), &
               rate, &
               t_routine_min(i), &
               t_routine_max(i)
            IF(lwritetime)&
               WRITE(iotiming,1) t_routine_ave(i)*100. / t_routine_ave(section%nroutine+2), &
               section%aroutine(i), &
               t_routine_ave(i) / section%timer_routine%icall(i), &
               section%timer_routine%icall(i), &
               rate, &
...
!***********************************************************************
      ...
      isec = id_timing_sec(sec_temp,nsection,section_cum)
...
#ifdef CUDA
      if(.not. ltiming_routine_i(isub))then
         ltiming_routine_i(isub)=.true.
...
!***********************************************************************
...
#ifdef CUDA
      if(.not. ltiming_routine_e(isub))then
         ltiming_routine_e(isub)=.true.
      ...
      ttt=ttt/1000.0
#else
      ttt = current_time() - timing_routine(isub)
#endif
      ...
      section_cum(isec)%timer_routine%mintiming(isub) = min(timing_routine(isub), &
         section_cum(isec)%timer_routine%mintiming(isub))
      section_cum(isec)%timer_routine%maxtiming(isub) = max(timing_routine(isub), &
         section_cum(isec)%timer_routine%maxtiming(isub))
      section_blk(isec)%timer_routine%mintiming(isub) = min(timing_routine(isub), &
         section_blk(isec)%timer_routine%mintiming(isub))
      section_blk(isec)%timer_routine%maxtiming(isub) = max(timing_routine(isub), &
         section_blk(isec)%timer_routine%maxtiming(isub))

      timer_standalone(isub)%icall = timer_standalone(isub)%icall + 1
      timer_standalone(isub)%timing = timer_standalone(isub)%timing + &
         timing_routine(isub)
      timer_standalone(isub)%sqtiming = timer_standalone(isub)%sqtiming + &
         timing_routine(isub)*timing_routine(isub)
      timer_standalone(isub)%mintiming = section_cum(isec)%timer_routine%mintiming(isub)
      timer_standalone(isub)%maxtiming = section_cum(isec)%timer_routine%maxtiming(isub)
   ...
   FUNCTION id_timing_sec(secname,n,mysection_cum)
      CHARACTER(len=FUNCTION_NAME_LEN) :: secname
      INTEGER :: n
      type(section_t), dimension(n) :: mysection_cum
      INTEGER :: id_timing_sec
      INTEGER :: i

      id_timing_sec = -99
      DO i=1, n
         IF(trim(mysection_cum(i)%name_sec)==trim(secname)) THEN
            id_timing_sec = i
            RETURN
         ENDIF
      ENDDO
...
!***********************************************************************
      ...
      write(iu,'(4a)')trim(mybanner2)," = ",trim(adjustl(r_char2))," (MB)"
      write(iu,of)"                                                                               "
      write(iu,of)"*******************************************************************************"
      write(iu,of)"                                                                               "
...
!***********************************************************************
      ...
      fout = real(total-myfree,kind=PRC)/(1024.0**3.0)
      fout2 = real(total,kind=PRC)/(1024.0**3.0)
...
!***********************************************************************
      ...
      if(nprocs>1)then
         myd(1)=fout
         call MPI_ALLREDUCE(myd,myd2,1,MPI_MYFLOAT, &
            MPI_SUM,MPI_COMM_WORLD,ier)
         fout=myd2(1)
      endif
#endif
